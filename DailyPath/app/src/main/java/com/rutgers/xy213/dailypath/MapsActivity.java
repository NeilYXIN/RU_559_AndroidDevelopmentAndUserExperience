package com.rutgers.xy213.dailypath;
...
public class MapsActivity extends FragmentActivity implements OnMapReadyCallback,GoogleApiClient.OnConnectionFailedListener,GoogleApiClient.ConnectionCallbacks, com.google.android.gms.location.LocationListener, OnCompleteListener<Void> {
   ...
   // private PendingGeofenceTask mPendingGeofenceTask = PendingGeofenceTask.NONE;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (mGoogleApiClient == null) {
            mGoogleApiClient = new GoogleApiClient.Builder(this)
                    ...
                    .build();
            //Toast.makeText(getApplication(), "connected in maps", Toast.LENGTH_SHORT).show();
        }
     ...
     * This callback is triggered when the map is ready to be used.
     */
    public void getdialog(final LatLng latLng1){
        final EditText et=new EditText(this);

        new AlertDialog.Builder(this).setTitle("Enter name").setView(et).
                setPositiveButton("Yes", new DialogInterface.OnClickListener() {
                    @Override
                    public void onClick(DialogInterface arg0, int arg1){
                        mMap.addMarker(new MarkerOptions()
                                .position(new LatLng(latLng1.latitude, latLng1.longitude)).snippet(et.getText().toString())
                        ...
                        parentaddress = "map";
                        mContentValues.put("name",et.getText().toString());//customized name
    ...
    public void onMapReady(GoogleMap googleMap) {
        ...
        Cursor mCursor = dbW.query("location", new String[]{"address","latitude","longitude"}, "marker=?", new String[]{"0"}, null, null, null);//query database
        if(mCursor.moveToFirst()){
            do{
                float latitude =Float.parseFloat(mCursor.getString(mCursor.getColumnIndex("latitude")));
                float longitude = Float.parseFloat(mCursor.getString(mCursor.getColumnIndex("longitude")));
    ...
    protected void startIntentService() {
        ...
        Intent intent1 = new Intent(this, GeofenceTransitionsIntentService.class);
    ...
    public void onLocationChanged(Location location) {
        ...
        // String currentlat = mLastLocation.getLatitude();
    ...
    class AddressResultReceiver extends ResultReceiver {
        ...
        protected void onReceiveResult(int resultCode, Bundle resultData) {
            ...
            mHandler.post(new Runnable() {
                @Override
                public void run() {
                    Toast.makeText(getApplication(), "address in maps is " + mAddressOutput, Toast.LENGTH_SHORT).show();
                }
            });
    ...
    private void initPopWindow() {
        ...
        if  (perth != null )
        {
            Log.i("Test!! currentfence is ", currentfence);
            Log.i("Test!!lastfence is ", lastfence);
            perth.hideInfoWindow();
        }

        if (!currentfence.equals("default") )
        {

            Cursor mCursor1 = dbW.query("location", new String[]{"time","name","address","latitude","longitude"}, "marker=? AND address=? AND named=?", new String[]{"0",currentfence,"1"}, null, null, null);//query database
            if(mCursor1.moveToFirst()){
                do{
                    Double latitude =Double.parseDouble(mCursor1.getString(mCursor1.getColumnIndex("latitude")));
                    Double longitude = Double.parseDouble(mCursor1.getString(mCursor1.getColumnIndex("longitude")));
                    String address = mCursor1.getString(mCursor1.getColumnIndex("address"));
                    String name = mCursor1.getString(mCursor1.getColumnIndex("name"));
                    String time = mCursor1.getString(mCursor1.getColumnIndex("time"));
                    final LatLng PERTH = new LatLng(latitude, longitude);
                    perth = mMap.addMarker(new MarkerOptions()
                            .position(PERTH).title(name).snippet(time)
                            .draggable(true));
